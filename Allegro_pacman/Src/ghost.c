...
Ghost* ghost_create(int flag) {
	...
	switch (character_type) {
		case 1:
			...
			break;
		case 2:
			ghost->flee_sprite = load_bitmap("Assets/iron-cross_flee.png");
			break;
		default:
			ghost->flee_sprite = load_bitmap("Assets/ghost_flee.png");
			break;
	...
	case Blinky:
		...
		switch (character_type) {
			case 1:
				...
				break;
			case 2:
				ghost->move_sprite = load_bitmap("Assets/iron-cross_move.png");
				break;
			default:
				ghost->move_sprite = load_bitmap("Assets/ghost_move_red.png");
				break;
		}
		ghost->move_script = &ghost_move_script_Blinky;
		break;
	case Pinky:
		...
		ghost->move_script = &ghost_move_script_Pinky;
		break;
	case Inky:
		ghost->objData.Coord.x = cage_grid_x;
		ghost->objData.Coord.y = cage_grid_y;
		...
		ghost->move_script = &ghost_move_script_Inky;
		break;
	case Clyde:
		ghost->objData.Coord.x = cage_grid_x;
		ghost->objData.Coord.y = cage_grid_y;
		...
		ghost->move_script = &ghost_move_script_Clyde;
		break;
	default:
		ghost->objData.Coord.x = cage_grid_x;
		ghost->objData.Coord.y = cage_grid_y;
	...
	*/
	...
	if (ghost->status == FLEE) {
		...
		*/ 
			...
			if (get_power_up_timer_tick() * 100 / get_power_up_duration() >= 70)
			{
