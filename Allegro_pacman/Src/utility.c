...
bool pnt_in_rect(int px, int py, RecArea field) {
	if (px < field.x || px > field.x + field.w) {
		return false;
	}
	if (py < field.y || py > field.y + field.h) {
		return false;
	}
	return true;
}
...
High_score_table* create_high_score_table(const char* filepath) {
	High_score_table* H = (High_score_table*)malloc(sizeof(High_score_table));
	H->picture = load_bitmap("Assets/high_score_table.png");

	FILE* pFile = NULL;
	if (!H) {
		game_abort("Error when creating High_score_table");
		return NULL;
	}
	...
	if (filepath == NULL) {
		...
		if (fscanf(pFile, "%d", &(H->row_num)) != 1) {
			game_abort("Table format unmatched\n");
			return NULL;
		}

		for (int i = 0; i < H->row_num; i++) {
			char tmp[20];
			if (fscanf(pFile, "%s %d", tmp, &(H->records[i].score)) != 2) {
				game_abort("Table format unmatched\n");
				return NULL;
			}
			for (int j = 0; j < 20; j++) {
				H->records[i].name[j] = tmp[j];
			}
		}
...
void draw_high_score_table(const High_score_table* high_score_table) {
	al_draw_bitmap(high_score_table->picture, 100, 50, 0);
	al_draw_text(
		titleFont,
	...
	for (int i = 0; i < high_score_table->row_num; i++) {
		...
		sprintf_s(tmp, 30, "%2d. %s %3d", 
			i+1,
			high_score_table->records[i].name,
			high_score_table->records[i].score);
		al_draw_text(
			menuFont,
			al_map_rgb(255, 255, 0),
			SCREEN_W / 2, 150+55*i,
...
void store_score_records(const char* filepath, const High_score_table* H) {
	FILE* pFile = NULL;
	if (filepath == NULL) {
		return;
	}
		...
		fprintf(pFile, "%d\n", H->row_num);
		for (int i = 0; i < H->row_num; i++) {
			fprintf(pFile, "%s %d\n", H->records[i].name, H->records[i].score);
		}
	}
	if (pFile) fclose(pFile);
}
