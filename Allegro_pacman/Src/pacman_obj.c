...
static bool pacman_movable(const Pacman* pacman, const Map* M, Directions targetDirec) {
	...
	if (!pacman->can_cross_wall && (is_wall_block(M, x, y) || is_room_block(M, x, y)))
		return false;
...
Pacman* pacman_create() {
	...
	switch (character_type) {
		case 1:
			...
			break;
		case 2:
			pman->move_sprite = load_bitmap("Assets/Star_of_David_move.png");
			pman->die_sprite = load_bitmap("Assets/Star_of_David_dead.png");
			break;
		default:
			pman->move_sprite = load_bitmap("Assets/pacman_move.png");
			pman->die_sprite = load_bitmap("Assets/pacman_die.png");
	...
	*/
	...
	if (!game_over) {
		...
		*/
		...
		al_draw_scaled_bitmap(pman->die_sprite, 0 + al_get_timer_count(pman->death_anim_counter) * 16, 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0
		);
